...
const ALLOWED_ADMIN_EMAIL = "ibr@ibr.com";
const currentPath = window.location.pathname.split("/").pop() || "index.html";
const isLoginPage = currentPath === "login.html";
const protectedAdminPages = ["admin.html", "admin-messages.html", "admin-content.html", "admin-projects.html", "admin-gallery.html", "admin-branding.html", "admin-analytics.html"];
const isProtectedAdminPage = protectedAdminPages.includes(currentPath);
...
function isAllowedUser(session) {
  return !!session?.user?.email && session.user.email.toLowerCase() === ALLOWED_ADMIN_EMAIL;
}
...
async function handleLoginPage() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!isAllowedUser(data.session)) {
      ...
      if (errorBox) errorBox.textContent = "Access denied for this account.";
      if (loginBtn) loginBtn.textContent = "Sign in";
      return;
    }
    window.location.replace(DASHBOARD_PATH);
...
const db = window.db || window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
window.db = db;
...
function buildLocalizedKeys(lang) {
  return localizedTextKeys.map((key) => `${key}_${lang}`);
}
...
async function loadOverview() {
  ...
  const [projectsRes, galleryRes, linksRes, eventsRes, messagesRes] = await Promise.all([
  ...
  statsContainer.innerHTML = [
    ...
    ["Messages", messagesRes.count || 0, "ri-mail-line"],
...
async function loadSiteContent() {
  const defaultForm = document.getElementById("contentForm");
  const arabicForm = document.getElementById("contentFormAr");
  const turkishForm = document.getElementById("contentFormTr");
  const { data, error } = await db.from("site_content").select("*");
  if (error) throw error;

  const defaultMap = {};
  siteContentKeys.forEach((key) => { defaultMap[key] = ""; });

  const arabicMap = {};
  localizedTextKeys.forEach((key) => { arabicMap[key] = ""; });

  const turkishMap = {};
  localizedTextKeys.forEach((key) => { turkishMap[key] = ""; });

  (data || []).forEach((row) => {
    if (!row.content_key) return;
    if (siteContentKeys.includes(row.content_key)) defaultMap[row.content_key] = row.content_value;
    if (row.content_key.endsWith('_ar')) {
      const baseKey = row.content_key.slice(0, -3);
      if (localizedTextKeys.includes(baseKey)) arabicMap[baseKey] = row.content_value || "";
    }
    if (row.content_key.endsWith('_tr')) {
      const baseKey = row.content_key.slice(0, -3);
      if (localizedTextKeys.includes(baseKey)) turkishMap[baseKey] = row.content_value || "";
    }
  });

  fillForm(defaultForm, defaultMap);
  fillForm(arabicForm, arabicMap);
  fillForm(turkishForm, turkishMap);
}
...
async function saveLocalizedSiteContent(event, lang) {
  event.preventDefault();
  const formData = new FormData(event.target);
  const payload = localizedTextKeys.map((key) => ({
    content_key: `${key}_${lang}`,
    content_value: formData.get(key) || "",
    is_visible: true
  }));
  const { error } = await db.from("site_content").upsert(payload, { onConflict: "content_key" });
  if (error) throw error;
  showToast(lang === 'ar' ? "Arabic texts saved successfully." : "Turkish texts saved successfully.");
}
...
async function loadFaviconSettings() {
  ...
  if (!form) return;
  const { data, error } = await db.from("site_content").select("content_key, content_value").eq("content_key", "site_favicon_url").maybeSingle();
  if (error && !String(error.message || '').toLowerCase().includes('no rows')) throw error;
  const value = data?.content_value || "";
  form.elements.namedItem("site_favicon_url").value = value;
...
async function uploadFaviconIfNeeded() {
  const fileInput = document.getElementById("siteFaviconFile");
  const file = fileInput?.files?.[0];
  if (!file) return null;
  const safeName = file.name.replace(/\s+/g, "-");
  const path = `branding/favicon-${Date.now()}-${safeName}`;
  const { error } = await db.storage.from("site-media").upload(path, file, { upsert: true });
  if (error) throw error;
  const { data } = db.storage.from("site-media").getPublicUrl(path);
  return data.publicUrl;
...
async function saveFaviconSettings(event) {
  event.preventDefault();
  const form = event.target;
  const uploaded = await uploadFaviconIfNeeded();
  const value = uploaded || form.elements.namedItem("site_favicon_url").value || "";
  const { error } = await db.from("site_content").upsert([{
    content_key: "site_favicon_url",
    content_value: value,
    is_visible: true
  }], { onConflict: "content_key" });
  if (error) throw error;
  ...
  if (document.getElementById("siteFaviconFile")) document.getElementById("siteFaviconFile").value = "";
  showToast("Favicon saved successfully.");
}

function previewFaviconFile() {
  const input = document.getElementById("siteFaviconFile");
  const preview = document.getElementById("siteFaviconPreview");
  const file = input?.files?.[0];
  if (!file || !preview) { if (preview) preview.hidden = true; return; }
  const reader = new FileReader();
...
async function initializeDashboard() {
  const tasks = [];
  if (document.getElementById("dashboardStats")) tasks.push(loadOverview());
  if (document.getElementById("contentForm") || document.getElementById("contentFormAr") || document.getElementById("contentFormTr")) tasks.push(loadSiteContent());
  if (document.getElementById("projectsTable") || document.getElementById("projectForm")) tasks.push(loadProjects());
  if (document.getElementById("logoForm")) tasks.push(loadLogoSettings());
  if (document.getElementById("faviconForm")) tasks.push(loadFaviconSettings());
  if (document.getElementById("galleryTable") || document.getElementById("galleryForm")) tasks.push(loadGallery());
  if (document.getElementById("linksTable") || document.getElementById("linkForm")) tasks.push(loadLinks());
  if (document.getElementById("topPages") || document.getElementById("recentVisits")) tasks.push(loadAnalytics());
...
function registerEvents() {
  ...
  document.getElementById("contentFormAr")?.addEventListener("submit", async (event) => {
    try { await saveLocalizedSiteContent(event, "ar"); } catch (error) { showToast(normalizeError(error)); }
  });
  document.getElementById("contentFormTr")?.addEventListener("submit", async (event) => {
    try { await saveLocalizedSiteContent(event, "tr"); } catch (error) { showToast(normalizeError(error)); }
  });
  ...
  document.getElementById("faviconForm")?.addEventListener("submit", async (event) => {
    try { await saveFaviconSettings(event); } catch (error) { showToast(normalizeError(error)); }
  });
  ...
  document.getElementById("resetFaviconForm")?.addEventListener("click", async () => {
    const form = document.getElementById("faviconForm");
    if (!form) return;
    form.reset();
    const preview = document.getElementById("siteFaviconPreview");
    if (preview) { preview.hidden = true; preview.removeAttribute("src"); }
    try { await loadFaviconSettings(); } catch (error) { showToast(normalizeError(error)); }
  });
  document.getElementById("removeFaviconBtn")?.addEventListener("click", async () => {
    try { await removeFavicon(); } catch (error) { showToast(normalizeError(error)); }
  });
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    if (connection) connection.textContent = "Connection issue";
    const errorBox = document.getElementById("adminLoginError");
    if (errorBox) errorBox.textContent = normalizeError(error);
...
document.addEventListener("DOMContentLoaded", () => {
  loadMessagesPage().catch(console.error);
  ...
  document.getElementById("siteFaviconFile")?.addEventListener("change", previewFaviconFile);
  document.getElementById("galleryFile")?.addEventListener("change", previewGalleryFile);
});
...
async function removeFavicon() {
  ...
  showToast("Favicon removed. Default icon restored.");
}
...
async function loadMessagesPage() {
  ...
  if (error) {
    wrap.innerHTML = `<p>${error.message}</p>`;
    return;
  }
