...
const ADMIN_EMAIL = "ibrahimmasoud201@gmail.com";
...
function setDashboardVisible(isVisible) {
  document.getElementById("adminLoginScreen")?.toggleAttribute("hidden", isVisible);
  const shell = document.getElementById("adminShell");
  if (shell) shell.hidden = !isVisible;
}

function bindAdminAuthUI() {
...
async function handleAdminLogin(event) {
  ...
  if (!db) {
    if (errorBox) errorBox.textContent = "راجع إعدادات Supabase أولاً.";
    return;
  }
  if (!email || !password) {
  ...
  if (email.toLowerCase() !== ADMIN_EMAIL) {
    if (errorBox) errorBox.textContent = "Access is allowed only for your account.";
    return;
  }
  ...
  try {
    ...
    if (!data?.user || String(data.user.email || "").toLowerCase() !== ADMIN_EMAIL) {
...
async function initAdminAuth() {
  ...
  if (!db) {
    const errorBox = document.getElementById("adminLoginError");
    if (errorBox) errorBox.textContent = "راجع Project URL و Publishable Key في supabase-config.js";
    setDashboardVisible(false);
    return;
  }
  try {
