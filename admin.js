...
async function loadOverview() {
  ...
  const [projectsRes, galleryRes, linksRes, eventsRes] = await Promise.allSettled([
  ...
  const countOf = (result) => result.status === "fulfilled" && !result.value.error ? (result.value.count || 0) : 0;
  ...
  statsContainer.innerHTML = [
    ["Projects", countOf(projectsRes), "ri-stack-line"],
    ["Gallery", countOf(galleryRes), "ri-image-line"],
    ["Links", countOf(linksRes), "ri-links-line"],
    ["Page Views", countOf(eventsRes), "ri-bar-chart-line"],
...
async function initializeDashboard() {
  ...
  const firstRejected = tasks.find((item) => item.status === "rejected");
  if (firstRejected) console.warn(firstRejected.reason);
}
...
function registerEvents() {
  ...
  document.getElementById("removeLogoBtn")?.addEventListener("click", async () => {
    try { await removeLogoSettings(); } catch (error) { showToast(normalizeError(error)); }
  });
