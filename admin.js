...
const db = (window.supabase && SUPABASE_URL && SUPABASE_ANON_KEY) ? window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY) : null;
...
function ensureDb() {
  if (!db) throw new Error("Supabase is not configured. Check supabase-config.js");
  return db;
}
...
async function removeLogo() {
  ...
  if (!confirm("Remove the current logo and restore IBR text?")) return;

  const activeDb = ensureDb();
  const { error } = await activeDb.from("site_content").upsert([{
    content_key: "site_logo_url",
    content_value: "",
    is_visible: true
  }], { onConflict: "content_key" });
  ...
  if (form) form.elements.namedItem("site_logo_url").value = "";
  if (document.getElementById("siteLogoFile")) document.getElementById("siteLogoFile").value = "";
  if (preview) {
    preview.hidden = true;
    preview.removeAttribute("src");
