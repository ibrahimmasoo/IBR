...
function ensureAuth(actionName) {
  if (!window.currentSession) {
    showToast(`Login first to ${actionName}.`);
    return false;
  }
  return true;
}
...
async function saveSiteContent(event) {
  ...
  if (!ensureAuth("save content")) return;
  ...
  if (error) throw error;
  showToast("Website text content saved.");
}
...
async function saveProject(event) {
  event.preventDefault();
  if (!ensureAuth("save project")) return;
...
async function handleProjectsTableClick(event) {
  ...
  if (!button) return;
  if (!ensureAuth("manage projects")) return;
...
async function saveGallery(event) {
  event.preventDefault();
  if (!ensureAuth("save image")) return;
...
async function handleGalleryTableClick(event) {
  const button = event.target.closest("button[data-action]");
  if (!button) return;
  if (!ensureAuth("manage gallery")) return;
...
async function saveLink(event) {
  event.preventDefault();
  if (!ensureAuth("save link")) return;
...
async function handleLinksTableClick(event) {
  const button = event.target.closest("button[data-action]");
  if (!button) return;
  if (!ensureAuth("manage links")) return;
