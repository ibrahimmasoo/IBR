...
function setDashboardVisibility(isLoggedIn) {
  const loginShell = document.getElementById("adminLoginShell");
  const adminShell = document.getElementById("adminShell");
  if (loginShell) loginShell.hidden = isLoggedIn;
  if (adminShell) adminShell.hidden = !isLoggedIn;
}
...
async function bootDashboard() {
  ...
  try {
    ...
    const status = document.getElementById("connectionStatus");
    if (status) status.textContent = "Connection issue";
...
async function applySessionState(session) {
  ...
  if (user && email !== ADMIN_EMAIL) {
    ...
    if (loginError) loginError.textContent = "This dashboard is restricted to your admin account only.";
  }
...
async function handleAdminLogin(event) {
  ...
  if (email !== ADMIN_EMAIL) {
    if (loginError) loginError.textContent = "Use your admin email only.";
    return;
  }
  ...
  try {
    ...
    if (loginError) loginError.textContent = normalizeError(error);
  } finally {
