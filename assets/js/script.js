...
const FALLBACK_SITE_CONTENT = {
  ...
  projects_page_description: "Explore all my projects in one place.",
  projects_view_more_text: "View More",
  project_button_text: "VIEW PROJECT",
  ...
  gallery_page_heading: "All Gallery",
  gallery_page_description: "Explore all images in one place.",
  gallery_view_more_text: "View More",
  gallery_back_home_text: "Back Home",
  ...
  support_button_text: "Donate",
  footer_text: "© 2026 Ibrahim Masood",
  chatbot_title: "IBR AI Assistant",
  chatbot_subtitle: "Ask me anything about Ibrahim",
  chatbot_welcome: "Hi! Ask me anything about Ibrahim.",
  chatbot_placeholder: "Ask about Ibrahim...",
...
const FALLBACK_GALLERY = [
  { title: "Gallery 1", image_url: "assets/assets/images/gallery-1.jpeg", alt_text: "Gallery Image 1", layout_type: "tall", is_visible: true, display_order: 1 },
  { title: "Gallery 2", image_url: "assets/assets/images/gallery-2.jpeg", alt_text: "Gallery Image 2", layout_type: "normal", is_visible: true, display_order: 2 }
];
...
function setLanguage(lang) {
  ...
  qsa(".lang-switcher button").forEach((btn) => {
...
function toggleTheme() {
  ...
  qsa(".theme-toggle").forEach((themeBtn) => {
...
function renderSiteFavicon() {
  ...
  favicon.href = faviconUrl || 'assets/images/ibrahim-masood-ibr-favicon.jpeg';
}
...
function renderSiteContent() {
  ...
  setText(".hero-content .btn", content.hero_cta_text);
  ...
  setText(".about-btn", content.about_cta_text);
  setText("body:not(.projects-page) #projects .projects-header h2", content.projects_heading);
  setText("body:not(.projects-page) #projects .projects-header p", content.projects_description);
  setText("body.projects-page .projects-section .projects-header h2", content.projects_page_heading || content.projects_heading);
  setText("body.projects-page .projects-section .projects-header p", content.projects_page_description || content.projects_description);
  setText("body:not(.gallery-page) #gallery .gallery-header h2", content.gallery_heading);
  setText("body:not(.gallery-page) #gallery .gallery-header p", content.gallery_description);
  setText("body.gallery-page .gallery-section .gallery-header h2", content.gallery_page_heading || content.gallery_heading);
  setText("body.gallery-page .gallery-section .gallery-header p", content.gallery_page_description || content.gallery_description);
  ...
  const projectViewMore = document.querySelector("#projects .view-more-link span") || document.querySelector("#projects .view-more-link");
  if (projectViewMore) {
    if (projectViewMore.tagName === 'SPAN') projectViewMore.textContent = content.projects_view_more_text;
    else projectViewMore.childNodes[0] && (projectViewMore.childNodes[0].textContent = `${content.projects_view_more_text} `);
  }
  const galleryViewMore = document.querySelector("#gallery .view-more-link span") || document.querySelector("#gallery .view-more-link");
  if (galleryViewMore) {
    if (galleryViewMore.tagName === 'SPAN') galleryViewMore.textContent = content.gallery_view_more_text;
    else galleryViewMore.childNodes[0] && (galleryViewMore.childNodes[0].textContent = `${content.gallery_view_more_text} `);
  }
  setText("body.gallery-page .gallery-section .btn", content.gallery_back_home_text);

  qsa(".project-btn").forEach((btn) => btn.textContent = content.project_button_text || "VIEW PROJECT");
  ...
  if (talkItems[0]) {
    ...
    if (label) label.textContent = content.contact_email_label || "EMAIL";
  }
  if (talkItems[1]) {
    if (content.contact_linkedin_url) talkItems[1].href = content.contact_linkedin_url;
    const label = talkItems[1].querySelector("span");
    if (label) label.textContent = content.contact_linkedin_label || "LINKEDIN";
  }

  setText('label[for="talkName"]', content.contact_form_name_label);
  const talkName = byId('talkName');
  if (talkName) talkName.placeholder = content.contact_form_name_placeholder || '';
  setText('label[for="talkEmail"]', content.contact_form_email_label);
  const talkEmail = byId('talkEmail');
  if (talkEmail) talkEmail.placeholder = content.contact_form_email_placeholder || '';
  setText('label[for="talkMessage"]', content.contact_form_message_label);
  const talkMessage = byId('talkMessage');
  if (talkMessage) talkMessage.placeholder = content.contact_form_message_placeholder || '';
  setText('.talk-btn', content.contact_form_button_text);

  setText("#support .projects-header h2", content.support_heading);
  setText("#support .projects-header p", content.support_description);
  qsa('#support .btn').forEach((btn) => btn.textContent = content.support_button_text || 'Donate');
  setText("footer", content.footer_text);

  setText('.chatbot-header h3', content.chatbot_title);
  setText('.chatbot-header p', content.chatbot_subtitle);
  setText('#chatbotMessages .bot-message', content.chatbot_welcome);
  const chatbotInput = byId('chatbotInput');
  if (chatbotInput) chatbotInput.placeholder = content.chatbot_placeholder || '';
  setText('#chatbotForm button', content.chatbot_send_text);
...
function projectCardMarkup(project) {
  ...
  return `
    <div class="project-card">
      ${project.image_url ? `<img src="${escapeHtml(project.image_url)}" alt="${escapeHtml(project.title)} by Ibrahim Masood IBR" title="${escapeHtml(project.title)} | Ibrahim Masood" loading="lazy" decoding="async">` : ''}
...
function bindCommonInteractions() {
  ...
  if (logoLink && !logoLink.dataset.boundRefresh) {
    logoLink.addEventListener("click", function (e) {
      e.preventDefault();
      const target = `index.html?refresh=${Date.now()}#home`;
      window.location.href = target;
    });
    logoLink.dataset.boundRefresh = "true";
  }
