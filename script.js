...
function toggleMobileMenu() {
  const navPanel = document.querySelector(".nav-panel");
  if (!navPanel) return;
  setMobileMenuState(!navPanel.classList.contains("active"));
}
...
function renderSiteLogo() {
  ...
  document.querySelectorAll(".logo").forEach((logo) => {
    ...
    const textNode = logo.querySelector(".site-logo-text");
    if (!img) return;
...
function bindCommonInteractions() {
  ...
  syncThemeButtons();
  qsa(".theme-toggle").forEach((themeBtn) => themeBtn.addEventListener("click", toggleTheme));
  ...
  if (menuToggle && navPanel) {
    ...
    navPanel.querySelectorAll("a, .lang-switcher button, .theme-toggle").forEach((el) => {
      el.addEventListener("click", () => {
        if (window.innerWidth <= 980 && (el.tagName === "A" || el.classList.contains("theme-toggle") || el.dataset.lang)) {
          setTimeout(() => setMobileMenuState(false), 50);
        }
      });
    });
