const translations = {
  en: {
    ...
    support: "Support",
    contact: "Contact",
    chatbotTitle: "IBR Assistant",
    chatbotSubtitle: "Quick answers about Ibrahim, his work, and how to reach him.",
    chatbotWelcome: "Hi! I can help you explore Ibrahim's projects, gallery, platforms, and contact details.",
    chatbotPlaceholder: "Ask about Ibrahim...",
    chatbotSend: "Send",
    chatbotSuggestion1: "What projects should I see first?",
    chatbotSuggestion2: "How can I contact Ibrahim?",
    chatbotSuggestion3: "Show me Ibrahim's platforms",
    chatbotSuggestion4: "What is in the gallery?",
  ...
  ar: {
    ...
    support: "الدعم",
    contact: "تواصل",
    chatbotTitle: "مساعد IBR",
    chatbotSubtitle: "إجابات سريعة عن إبراهيم وأعماله وطرق التواصل معه.",
    chatbotWelcome: "مرحبًا! أستطيع مساعدتك في استكشاف مشاريع إبراهيم والمعرض والمنصات وطرق التواصل.",
    chatbotPlaceholder: "اسأل عن إبراهيم...",
    chatbotSend: "إرسال",
    chatbotSuggestion1: "ما أهم المشاريع التي أبدأ بها؟",
    chatbotSuggestion2: "كيف أتواصل مع إبراهيم؟",
    chatbotSuggestion3: "اعرض منصات إبراهيم",
    chatbotSuggestion4: "ماذا يوجد في المعرض؟",
  ...
  tr: {
    ...
    support: "Destek",
    contact: "İletişim",
    chatbotTitle: "IBR Asistan",
    chatbotSubtitle: "İbrahim, çalışmaları ve iletişim yolları hakkında hızlı yanıtlar.",
    chatbotWelcome: "Merhaba! İbrahim'in projelerini, galerisini, platformlarını ve iletişim bilgilerini keşfetmene yardımcı olabilirim.",
    chatbotPlaceholder: "İbrahim hakkında sor...",
    chatbotSend: "Gönder",
    chatbotSuggestion1: "İlk hangi projeleri görmeliyim?",
    chatbotSuggestion2: "İbrahim'e nasıl ulaşabilirim?",
    chatbotSuggestion3: "İbrahim'in platformlarını göster",
    chatbotSuggestion4: "Galeride neler var?",
...
function getChatbotUiStrings(lang = state.currentLang || 'en') {
  const t = translations[lang] || translations.en;
  return {
    title: t.chatbotTitle || 'IBR Assistant',
    subtitle: t.chatbotSubtitle || 'Quick answers about Ibrahim, his work, and how to reach him.',
    welcome: t.chatbotWelcome || "Hi! I can help you explore Ibrahim's projects, gallery, platforms, and contact details.",
    placeholder: t.chatbotPlaceholder || 'Ask about Ibrahim...',
    send: t.chatbotSend || 'Send',
    suggestions: [t.chatbotSuggestion1, t.chatbotSuggestion2, t.chatbotSuggestion3, t.chatbotSuggestion4].filter(Boolean)
  };
}

function renderChatbotUi() {
  const ui = getChatbotUiStrings();
  setText('#chatbotTitle', ui.title);
  setText('#chatbotSubtitle', ui.subtitle);
  setText('#chatbotWelcomeMessage', ui.welcome);
  const input = byId('chatbotInput');
  if (input) input.placeholder = ui.placeholder;
  setText('#chatbotSendBtn', ui.send);

  const suggestionsWrap = byId('chatbotSuggestions');
  if (suggestionsWrap) {
    suggestionsWrap.innerHTML = ui.suggestions.map((question) =>
      `<button type="button" class="chatbot-suggestion">${escapeHtml(question)}</button>`
    ).join('');
  }
}

function getChatbotReply(rawMsg) {
  const msg = String(rawMsg || '').toLowerCase();
  const lang = state.currentLang || 'en';
  const isAr = lang === 'ar';
  const isTr = lang === 'tr';

  if (msg.includes('project') || msg.includes('projects') || msg.includes('مشروع') || msg.includes('المشاريع') || msg.includes('proje')) {
    return isAr ? 'تقدر تبدأ من قسم المشاريع لاستكشاف أبرز أعمال إبراهيم، ثم تضغط على المشروع الذي يعجبك لمعرفة تفاصيل أكثر.'
      : isTr ? 'Önce Projeler bölümünden İbrahim'in öne çıkan çalışmalarını inceleyebilir, ardından ilgini çeken projeye tıklayarak daha fazla detay görebilirsin.'
      : "Start with the Projects section to explore Ibrahim's featured work, then open any project that interests you for more details.";
  }
  if (msg.includes('gallery') || msg.includes('معرض') || msg.includes('الصور') || msg.includes('galeri')) {
    return isAr ? 'قسم المعرض يعرض لقطات مختارة من أعمال إبراهيم ولحظاته وصوره الإبداعية بشكل بصري بسيط.'
      : isTr ? 'Galeri bölümü, İbrahim'in çalışmalarından, anlarından ve yaratıcı görsellerinden seçilmiş kareleri gösterir.'
      : 'The gallery section shows selected visuals, moments, and creative shots from Ibrahim's work and journey.';
  }
  if (msg.includes('contact') || msg.includes('email') || msg.includes('تواصل') || msg.includes('بريد') || msg.includes('iletişim') || msg.includes('e-posta')) {
    const content = getLocalizedSiteContent(lang);
    const email = content.contact_email || 'ibr@ibr.com';
    return isAr ? `تقدر تتواصل مع إبراهيم من قسم Let's Talk أو مباشرة عبر البريد: ${email}`
      : isTr ? `İbrahim'e Let's Talk bölümünden veya doğrudan şu e-posta adresinden ulaşabilirsin: ${email}`
      : `You can reach Ibrahim from the Let's Talk section or directly by email at ${email}.`;
  }
  if (msg.includes('platform') || msg.includes('linkedin') || msg.includes('github') || msg.includes('منصات') || msg.includes('platf')) {
    return isAr ? 'افتح قسم Platforms للوصول السريع إلى حسابات إبراهيم وروابطه على المنصات المختلفة.'
      : isTr ? 'Farklı platformlardaki hesaplarına hızlı erişim için Platforms bölümünü açabilirsin.'
      : 'Open the Platforms section for quick access to Ibrahim's accounts and links across different platforms.';
  }
  if (msg.includes('who') || msg.includes('about') || msg.includes('من هو') || msg.includes('مين') || msg.includes('kim') || msg.includes('hakkında')) {
    return isAr ? 'إبراهيم أشرف إبراهيم مسعود، المعروف أيضًا باسم IBR، هو طالب هندسة حاسبات مصري يشارك أعماله ومشاريعه ومسيرته الإبداعية عبر هذا الموقع.'
      : isTr ? 'IBR olarak da bilinen Ibrahim Ashraf Ibrahim Masood, çalışmalarını, projelerini ve yaratıcı yolculuğunu bu sitede paylaşan Mısırlı bir bilgisayar mühendisliği öğrencisidir.'
      : 'Ibrahim Ashraf Ibrahim Masood, also known as IBR, is an Egyptian Computer Engineering student who shares his work, projects, and creative journey through this site.';
  }
  return isAr ? 'اسألني عن المشاريع أو المعرض أو المنصات أو طرق التواصل مع إبراهيم.'
    : isTr ? 'Bana projeler, galeri, platformlar veya İbrahim ile iletişim hakkında soru sorabilirsin.'
    : "Ask me about Ibrahim's projects, gallery, platforms, or how to contact him.";
}
...
function bindCommonInteractions() {
  ...
  if (chatbotToggle && chatbotClose && chatbotBox && chatbotForm && chatbotInput && chatbotMessages) {
    ...
    chatbotToggle.addEventListener("click", () => {
      ...
      if (chatbotBox.classList.contains("open")) renderChatbotUi();
    });
    ...
    chatbotForm.addEventListener("submit", (e) => {
      ...
      chatbotInput.value = "";
      const reply = getChatbotReply(rawMsg);
      setTimeout(() => addMessage(reply, "bot", chatbotMessages), 220);
    });

    chatbotBox.addEventListener("click", (e) => {
      const suggestion = e.target.closest('.chatbot-suggestion');
      if (!suggestion) return;
      chatbotInput.value = suggestion.textContent.trim();
